...
1. **~~Wire `krunner osi` to write to the DB.~~ DONE 2026-06-17.** `krunner osi` now persists its enriched results via the new `KospexDependencies.save_dependencies(records, source=...)` method, called after the deps.dev enrichment loop (`src/krunner.py`). Each parsed record carries the commit `hash` from `file_metadata`, a canonical lowercase `package_type` (`pypi`/`npm`, matching `assess()` so rows reconcile rather than duplicate), and a mapped `package_use`. Rows are stamped `source = "krunner osi"`. `krunner osi -all` is now the canonical refresh command for `/dependencies/`.
...
4. **~~Fix `latest` semantics.~~ DONE 2026-06-17.** `save_dependencies()` demotes prior rows for each `(_repo_id, file_path, package_name)` to `latest=0` before upserting the incoming rows with `latest=1`, and the `latest=1` filter in `get_dependencies()` (`kospex_query.py`) is re-enabled. This is safe because only the demoting write path sets `latest=0`; repos that still only have legacy `sca` rows (which never demoted) are unaffected, since those rows are all `latest=1`. **Caveat:** `assess()`/`sca` still does NOT demote (it sets `latest=1` on every insert), so a repo scanned only via `sca` across multiple package versions can still show duplicates until re-run through `krunner osi` or until step 5 routes `assess()` through `save_dependencies()`.
...
- **~~Endpoints have not been verified end-to-end.~~ Data layer verified 2026-06-17.** Ran `krunner osi github.com~kospex~kospex` against the real DB: 72 rows written with `source='krunner osi'`, `latest=1`, populated `versions_behind`/`advisories`, and `_git_*` derived. Confirmed the demotion fired (prior `sca` rows for the repo dropped from `latest=1`) and that `get_dependencies()` (the `/dependencies/` data source) returns only current rows. **Distinction worth keeping straight:** `/osi/` reads `TBL_FILE_METADATA` (the dependency *files*) and is refreshed by `kospex sync`, NOT by this work — if `/osi/` looks stale, the fix is re-syncing the repos, not `krunner osi`. `krunner osi` → DB only feeds `/dependencies/` (the parsed packages + advisories). The HTML pages themselves (full `kweb2` render) were not loaded in a browser during this pass — still worth a manual check before removing the WIP banner (step 6).
