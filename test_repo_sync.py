...
Examples:
  ...
  python test_repo_sync.py /path/to/repo --db-path /custom/path/test.db
    ...
    parser.add_argument("repo_path", help="Path to the git repository to sync")

    parser.add_argument(
        "--db-path", help="Path to SQLite database file (default: ~/kospex/repo_sync_spike.db)"
    )

    parser.add_argument("--limit", type=int, help="Maximum number of commits to sync")

    parser.add_argument("--from-date", help="Start date for sync (ISO format, e.g., 2024-01-01)")

    parser.add_argument("--to-date", help="End date for sync (ISO format, e.g., 2024-12-31)")

    parser.add_argument("--no-scc", action="store_true", help="Disable scc file metadata analysis")
