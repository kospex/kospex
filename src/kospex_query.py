...
class KospexQuery:
    ...
    def get_kospex_db_version(self):
        ...
        """
        version_sql = (
            f"SELECT value FROM {KospexSchema.TBL_KOSPEX_CONFIG} where key = ? AND latest = 1"
        )
    ...
    def commit_history(self, repo_id: str, group_by: str = 'year'):
        """
        Get commit counts grouped by year or month for a repository.

        Args:
            repo_id: Repository identifier
            group_by: 'year' or 'month' for grouping (default: 'year')

        Returns:
            List of dictionaries with time period and commit count

        Example:
            >>> kq = KospexQuery()
            >>> kq.commit_history('github.com~owner~repo', 'year')
            [{'year': '2023', 'commit_count': 145}, {'year': '2024', 'commit_count': 203}]
        """
        if group_by == 'month':
            time_format = '%Y-%m'
            time_field = 'month'
        ...
        sql = f"""
            SELECT
                strftime('{time_format}', committer_when) AS {time_field},
    ...
    @KospexUtils.timer()
    def developers(
        self, org_key=None, repo_id=None, server=None, days=None, to_date=None, from_date=None
    ):
        """
        ...
        for i in results:
            ...
            i["tenure"] = KospexUtils.days_between_datetimes(
    ...
    @KospexUtils.timer()
    def authors(self, days=None, org_key=None, request_id=None):
        """
    ...
    def commit_ranges2(self, repo_id=None, org_key=None):
        ...
        ages = {"Active": 0, "Aging": 0, "Stale": 0, "Unmaintained": 0}
    ...
    def summarise_dev_commits(self, commits):
        ...
        """
        # Group by (author_email, language)
        grouped = defaultdict(lambda: {"commits": [], "repo_ids": set(), "file_paths": set()})
    ...
    def get_file_authors(self, file_name=None, repo_id=None):
        ...
        kd.select("file_path")
        kd.select_as("count(*)", "commits")
        kd.select_latest_date("commit_files.committer_when", "committer_when")
        kd.where("commits._repo_id", "=", repo_id)
        ...
        # Ordering on the already-normalised UTC alias, not the raw column.
    ...
    def get_single_observation(
        self, repo_id=None, observation_key=None, hash=None, file_path=None, uuid=None
    ):
    ...
    def add_observation(self, observation):
        ...
        self.kospex_db.table(KospexSchema.TBL_OBSERVATIONS).insert(
    ...
    def key_person(self, days=None, repo_id=None, top=None):
        ...
        for a in authors:
            ...
            days = KospexUtils.days_between_datetimes(
                a["first_commit"], a["last_commit"], min_one=True
            )
    ...
    def get_graph_info(
        self, org_key=None, author_email=None, repo_id=None, git_server=None, by_repo=None
    ):
        """
    ...
    def get_repo_files_graph_info(self, repo_id):
        ...
        kd.where("commits._repo_id", "=", repo_id)
        kd.where_join(
            KospexSchema.TBL_COMMITS, "_repo_id", KospexSchema.TBL_COMMIT_FILES, "_repo_id"
        )
...
class KospexData:
    ...
    # Aggregate a timestamp column on the true instant, then format the result
    # back to ISO-8601 in UTC.
    #
    # Commit dates are stored as ISO-8601 text carrying each committer's local
    # UTC offset, and text comparison of ISO-8601 is only valid when every value
    # shares one offset. kospex has 33 distinct offsets, so plain
    ...
    # committer's local offset. The raw column keeps the original offset.
    _UTC_ISO = "%Y-%m-%dT%H:%M:%SZ"

    def _select_date_aggregate(self, function_name, column, alias):
        if not self.is_valid_sql_name(column):
            raise ValueError(f"Column '{column}' is not a valid SQL name")
        if not self.is_valid_sql_name(alias):
            raise ValueError(f"Alias '{alias}' is not a valid SQL name")
    ...
    def select_latest_date(self, column, alias):
        """Select the most recent value of a timestamp column, as ISO-8601 UTC.

        Use instead of select_as("MAX(committer_when)", ...) — see the note
        above on why text comparison of these values is wrong.
        """
        self._select_date_aggregate("MAX", column, alias)

    def select_earliest_date(self, column, alias):
        """Select the earliest value of a timestamp column, as ISO-8601 UTC."""
        self._select_date_aggregate("MIN", column, alias)

    def order_by_date(self, column, direction="DESC"):
        """Order by a timestamp column on the true instant, not on its text.

        Use instead of order_by() for any *_when column.
        """
