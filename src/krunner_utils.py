...
def generate_krunner_csv_filename(command_key, request_id):
    """
    Generates a filename based on the key and the request_id
    E.g.
    generate_krunner_csv_filename("krunner-osi","github.com")
    filename: krunner-osi-github.com.csv

    generate_krunner_csv_filename("krunner-osi")
    This could also be that request_id is None
    file: krunner-osi.csv
    """
    filename = f"{command_key}.csv"
    if request_id:
        filename = f"{command_key}-{request_id}.csv"
    return filename
...
def find_js_libraries(file_path):
    ...
    for line_number, full_url, filename, version in results:
        print(
            f"{file_path}, Line {line_number}: Full URL: {full_url}, Filename: {filename}, version: {version}"
        )
...
def find_docker_compose_images_recursively(data, filename, parent_key=""):
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            if key == "image":
                image = {"base_image": value, "type": parent_key, "filename": filename}
                images.append(image)
            else:
                images.extend(
                    find_docker_compose_images_recursively(
                        value, filename, parent_key if parent_key else key
                    )
                )
